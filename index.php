		...
		<div class="card">
			...
			<table class="table">
  				...
  				<tbody>
    				<tr>
      					<th scope="row">Critical</th>
      					<td><?php $sum=mysqli_query($connection, "SELECT count(*) FROM app_vulns WHERE severity = 'Critical'"); while ($row = $sum->fetch_assoc()) {echo $row['count(*)']."<br>";} ?> </td>
    				</tr>
    				<tr>
      					<th scope="row">High</th>
      					<td><?php $sum=mysqli_query($connection, "SELECT count(*) FROM app_vulns WHERE severity = 'High'"); while ($row = $sum->fetch_assoc()) {echo $row['count(*)']."<br>";} ?> </td>
    				</tr>
    				<tr>
      					<th scope="row">Medium</th>
      					<td><?php $sum=mysqli_query($connection, "SELECT count(*) FROM app_vulns WHERE severity = 'Medium'"); while ($row = $sum->fetch_assoc()) {echo $row['count(*)']."<br>";} ?> </td>
    				</tr>
					<tr>
      					<th scope="row">Low</th>
      					<td><?php $sum=mysqli_query($connection, "SELECT count(*) FROM app_vulns WHERE severity = 'Low'"); while ($row = $sum->fetch_assoc()) {echo $row['count(*)']."<br>";} ?> </td>
    				</tr>
  				</tbody>
			</table>
	   </div>
